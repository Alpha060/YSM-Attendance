...
<body>
    ...
    const exportReport = (format: 'csv' | 'excel' | 'pdf') => {
        // Determine if filters are applied to single out a specific department & semester
        const isFiltered = !!selectedDepartmentId && !!selectedSemester;

        // Group subjects helper
        ...
        // Union with page-level available subjects if filtered
        if (isFiltered) {
            availableSubjects.forEach(sub => {
                if (!uniqueSubjectsMap.has(sub.id)) {
                    uniqueSubjectsMap.set(sub.id, {
                        id: sub.id,
                        name: sub.name,
                        paperCode: (sub as any).paperCode || null
                    });
                }
            });
        }
        ...
        const filename = `student_attendance_report_${new Date().toISOString().split('T')[0]}`;        if (format === 'excel') {
            ...
            if (isFiltered) {
                ...
                const sheetData = buildSheetData(filteredStudents, subjects, meta);
            ...
            const allSubjectsMap = new Map<string, { id: string; name: string; paperCode: string | null }>();
            filteredStudents.forEach(st => {
                st.subjectAttendance?.forEach(sub => {
            ...
            const headers = ['Department', 'Semester', 'Student ID', 'Roll Number', 'Name', ...allSubjects.map(sub => sub.paperCode ? `${sub.name} (${sub.paperCode})` : sub.name), 'Total Classes', 'Attended', 'Percentage', 'Status'];
            ...
            if (isFiltered) {
                tablesHtml = buildTableHtml(filteredStudents, subjects, `
                    <p class="meta">
                        <strong>Filters Applied:</strong> Generated on: ${new Date().toLocaleDateString()} | Total Students: ${filteredStudents.length}
                        ...
                        <div class="group-section" style="margin-bottom: 30px;">
                            ...
                            ${buildTableHtml(group.students, groupSubjects, `
                                <p class="meta" style="margin-bottom: 8px;">
                                    Generated on: ${new Date().toLocaleDateString()} | Total Students: ${group.students.length}
                                </p>
                            `)}
                        </div>
                    `;
